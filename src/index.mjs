...
function buildTransitionMatrix(spec) {
  ...
  for (const s in spec.outputs) if (!(s in tm)) tm[s] = {};

  for (const t of spec.table) {
    if (!(t.state in tm)) tm[t.state] = {};

    if (!(t.next in tm)) tm[t.next] = {};

    if (t.read in tm[t.state]) {
      const { write, move, next } = tm[t.state][t.read];
      if (write === t.write && move === MOVE[t.move] && next === t.next) {
        console.warn(
          `Duplicate transition for state '${t.state}' and symbol '${t.read}'`
        );
      } else {
        throw new TMError(
          `Inconsistent transitions for state '${t.state}' and symbol '${t.read}'`,
          t.loc
        );
      }
    } else {
...
function topoSortStates(init, tm) {
  ...
  for (const state in tm) remaining[state] = 1;
  ...
  while (root) {
    ...
    while (stack.length > 0) {
      ...
      for (; i < adj.length; ++i) {
        if (!(adj[i] in remaining)) continue;
...
const TABLE = `<table class="uk-table uk-table-middle uk-table-divider uk-table-striped uk-table-hover uk-table-small">
    <thead>
        <tr>
...
const TRACE_EMPTY = `<tr class="empty"><td colspan="7">The trace is empty.</td></tr>`;
const TRACE_ROW = `<td class="uk-table-shrink number">{{stepnum}}</td>
    ...
    <td class="uk-table-shrink ctr">{{next}}</td>
    <td>{{tape}}</td>`;
...
const INSERT_OP =
  /^insert(Text|FromYank|FromDrop|FromPaste|FromPasteAsQuotation)$/;
...
class App {
  ...
  reportError(message, location) {
    ...
    if (length === -1) length = this.editorTextArea.value.length - start;
  ...
  read(index) {
    return (
      (index < 0 ? this.tapeLeft[-index - 1] : this.tapeRight[index]) ??
      this.spec.blank
    );
  }
  ...
  write(index, symbol) {
    if (index < 0) {
      if (symbol === this.spec.blank) delete this.tapeLeft[-index - 1];
      else this.tapeLeft[-index - 1] = symbol;
    } else {
      if (symbol === this.spec.blank) delete this.tapeRight[index];
      else this.tapeRight[index] = symbol;
    }
  }
  ...
  move(m) {
    ...
    if (isNaN(mn) || Math.abs(mn) > 1) throw RangeError(`invalid move ${m}`);
  ...
  get result() {
    return this.nextStep
      ? "running..."
      : this.spec.outputs[this.state] ?? "stuck";
  }

  changeState(state, prevTransition = null) {
    if (!state || !(state in this.transitionMatrix)) return;

    const prevState = this.state;
    prevTransition = prevTransition ?? this.nextStep;

    if (state === prevState && this.nextStep === prevTransition) return;
  ...
  stepBackward() {
    ...
    this.tapeRow.children.item(headCellIndex).firstChild.innerText =
      lastStep.read;
  ...
  update() {
    ...
    if (prevBlank && prevBlank !== spec.blank) {
      ...
      if (notify)
        UIkit.notification(
          `'${this.spec.blank}' is now the blank symbol. Occurrences of '${this.spec.blank}' on tape have been changed to '?'.`
        );
    }
    ...
    if (this.state === null || !(this.state in this.transitionMatrix)) {
      if (this.state !== null)
        UIkit.notification(
          "Current state does not exist anymore, reverting to initial state."
        );
  ...
  updateTable() {
    ...
    this.tableView.innerHTML = Mustache.render(TABLE, {
      symbols: symbols,
      states: topoSortStates(this.spec.init, this.transitionMatrix).map(
        (state) => ({
          label: state,
          init: state === this.spec.init,
          output: this.spec.outputs[state],
          transitions: symbols.map(
            (sym) =>
              this.transitionMatrix[state][sym]?.tableEntry ?? {
                read: sym,
                halt: true,
              }
          ),
  ...
  updateTableStyle() {
    ...
    for (const cell of this.tableView.querySelectorAll("td.transition"))
      cell.classList.toggle(
        "active",
        cell.dataset.state === this.state && cell.dataset.read === headSymbol
      );
  }
  ...
  updateGraph() {
    ...
    while (prevStates.length > 0 && states.length > 0) {
      if (prevStates[prevStates.length - 1] < states[states.length - 1]) {
        ...
        prevStates[prevStates.length - 1] > states[states.length - 1]
      ) {
    ...
    for (const state of states) changed.push(state);

    for (const state of prevStates) removed.push(state);
    ...
    for (const e of edges) {
      edgeMap[e].node.label = edgeMap[e].node.transitions
        .map((t) => `${t.read}↦${t.write} ${MOVE_NAME[t.move]}`)
        .join("\n");
    }
    ...
    while (prevEdges.length > 0 && edges.length > 0) {
      if (prevEdges[prevEdges.length - 1] < edges[edges.length - 1]) {
        ...
        const edgeId = edges.pop(),
          { label, transitions } = edgeMap[edgeId].node;
  ...
  updateGraphStyle(prevState, prevTransition) {
    ...
    if (this.state === prevState && nextTransition === prevTransition) return;

    const prevEdgeId = prevTransition
      ? `${prevTransition.state} ${prevTransition.next}`
      : null;
    const nextEdgeId = nextTransition
      ? `${nextTransition.state} ${nextTransition.next}`
      : null;

    const prevEdgeNode = prevTransition ? this.nodes.get(prevEdgeId) : null;
    const nextEdgeNode =
      nextEdgeId === prevEdgeId
        ? prevEdgeNode
        : nextTransition
        ? this.nodes.get(nextEdgeId)
        : null;
    ...
    if (nextEdgeNode !== prevEdgeNode) {
      if (prevEdgeNode) {
        changedNodes.push({
          id: prevEdgeId,
          label: prevEdgeNode.transitions
            .map((t) => `${t.read}↦${t.write} ${MOVE_NAME[t.move]}`)
            .join("\n"),
        });
    ...
    if (nextEdgeNode) {
      ...
      changedNodes.push({
        id: nextEdgeId,
        label: nextEdgeNode.transitions
          .map((t) =>
  ...
  pushToTrace(step) {
    ...
    if (this.trace.length === 1) this.traceTable.innerHTML = "";

    const row = document.createElement("tr");
    const tapeTexts = Array.from(this.tapeRow.children).map(
      (cell) => cell.firstChild.innerText
    );
    // tape = first hash to last hash
    let tape = tapeTexts.join("").replace(/^.*?#/, "").replace(/#.*?$/, "");
    // do current step
    if (step.write !== "#") {
      tape = tape.slice(0, this.head - 1) + step.write + tape.slice(this.head);
    }
    row.innerHTML = Mustache.render(TRACE_ROW, {
      __proto__: step,
      stepnum: this.trace.length,
      move: MOVE_NAME[step.move],
      tape: `#${tape}#`,
    });
    this.traceTable.appendChild(row);

    if (this.traceView.matches(".uk-active")) row.scrollIntoView(false);
  }

  popFromTrace() {
    const step = this.trace.pop();

    if (this.trace.length <= 0) this.traceTable.innerHTML = TRACE_EMPTY;
    else this.traceTable.removeChild(this.traceTable.lastChild);
  ...
  updateTape(resize = false) {
    const tapeCellCount = Math.max(
      5,
      1 + 2 * (Math.floor((this.tape.clientWidth / 2 - 16) / 32) + 2)
    );

    if (resize && tapeCellCount <= this.tapeRow.childElementCount) return;
  ...
  startTapeMove(m) {
    ...
    if (this.ongoingTapeMove) this.endTapeMove();
  ...
  endTapeMove() {
    if (!this.ongoingTapeMove) return;
    ...
    if (this.ongoingTapeMove === L) {
      ...
      movedCell.remove();
      movedCell.firstChild.innerText = this.read(
        this.tapeCellBase + tapeCellCount - 1
      );
  ...
  async perform(action) {
    ...
    switch (action) {
      case "load":
        ...
        if (file) {
          try {
            this.editorTextArea.value = this.editorHighlights.innerText =
              await file.text();
          } catch (err) {
            window.alert(
              `Could not load '${file.name}': ${err.message ?? err.name ?? err}`
            );
            return;
          }
      ...
      case "save":
        const blob = new Blob([this.editorTextArea.value], {
      ...
      case "expt":
        html2canvas(document.getElementById("trace-view-table")).then(
          (canvas) => {
            canvas.toBlob((blob) => {
              FileSaver.saveAs(blob, "TuringMachineTrace.png");
            });
          }
        );
        return;
      ...
      case "expd":
        html2canvas(document.getElementById("graph-view")).then((canvas) => {
          canvas.toBlob((blob) => {
      ...
      case "stop":
        ...
        else prevTransition = this.nextStep;
  ...
  start() {
    ...
    this.tapeHead.addEventListener("beforeinput", (ev) => {
      ...
      if (!INSERT_OP.test(ev.inputType)) return;
      ...
      iter.eachGraphemeCluster(text, (symbol) => {
        if (symbol === " ") symbol = this.spec.blank;

        if (WS.test(symbol)) return;
    ...
    this.tapeHead.onkeydown = (ev) => {
      ...
      switch (ev.key) {
        ...
        case "Delete":
          ...
          if (ev.key === "Backspace") this.move(L);
          break;
        case "ArrowLeft":
          this.move(L);
          break;
    ...
    const init = async () => {
      ...
      if (params.has("view")) {
        try {
          UIkit.tab(document.querySelector("body > nav > *[uk-tab]")).show(
            parseInt(params.get("view"))
          );
        } catch (_) {
